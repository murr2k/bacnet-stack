...
{
    if (current_suite &&
        current_suite->total_tests < current_suite->max_results) {
        TestResult *result =
            &current_suite->results[current_suite->total_tests - 1];
        result->passed = false;
...
void test_suite_run(
    TestSuite *suite, bool (*test_func)(void), const char *test_name)
{
...
void test_suite_report(TestSuite *suite, FILE *output)
{
    ...
    double elapsed =
        (double)(suite->end_time - suite->start_time) / CLOCKS_PER_SEC;
...
void test_suite_generate_xml(TestSuite *suite, const char *filename)
{
    ...
    for (int i = 0; i < suite->total_tests; i++) {
        TestResult *result = &suite->results[i];
        fprintf(
            fp, "  <testcase name=\"%s\" classname=\"%s\"", result->test_name,
            suite->suite_name);
        ...
        if (result->passed) {
            ...
            fprintf(fp, ">\n");
            fprintf(
                fp, "    <failure message=\"%s\"",
                result->error_message ? result->error_message : "Test failed");
            if (result->file_name && result->line_number > 0) {
                fprintf(
                    fp, " type=\"AssertionError\">%s:%d</failure>\n",
